...
## 🏗️ O que eu sei fazer de verdade

```yaml
# Além do CRUD — o que um dev pleno realmente entrega

arquitetura:
  - Separação em camadas (Controller → Service → Repository)
  - Clean Architecture em projetos Node.js e Java
  - APIs RESTful com versionamento e documentação (Swagger/OpenAPI)
  - Microsserviços com comunicação via REST e fila (RabbitMQ)

qualidade_de_codigo:
  - Testes unitários e de integração (Jest, JUnit, pytest)
  - Code review construtivo e documentação técnica
  - SOLID, DRY, KISS na prática
  - Git Flow, commits semânticos, PRs organizados

devops_na_pratica:
  - Containers Docker com multi-stage builds
  - Orquestração com Kubernetes (Deployments, Services, Ingress)
  - Pipelines CI/CD com GitHub Actions (build → test → deploy)
  - Infraestrutura como código com Terraform na AWS

ia_e_automacao:
  - Agentes autônomos com LangChain + OpenAI API
  - RAG (Retrieval-Augmented Generation) com bases vetoriais
  - Workflows automatizados com n8n (integrações entre sistemas)
...
☁️  AWS Solutions Architect Associate — em andamento
⚙️  Kubernetes CKA — em andamento
```
...
> *"Bons devs escrevem código que funciona. Ótimos devs escrevem código que outros conseguem entender, manter e evoluir."*
